...
- deduplicate real frame - which frame of a run of duplicates is the one that was really drawn. `first` is what a live recording does (the picture is drawn, then held until the next is ready) and is almost always right; `last` suits footage where the run ends on the real frame instead, which is what a variable framerate recording resampled to a fixed one can look like. only matters where runs vary in length - getting it wrong there makes motion speed up and slow down
