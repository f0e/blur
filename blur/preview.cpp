...
LRESULT CALLBACK c_preview::wnd_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
	...
	case WM_PAINT: {
		...
		if (!preview.drawing) {
			...
			Gdiplus::Image image(helpers::towstring(preview.preview_path.string()).data());
...
void c_preview::watch_preview() {
	...
	while (preview.preview_open) {
		if (std::filesystem::exists(preview.preview_path) && preview.hwnd != NULL) {
			auto write_time = std::filesystem::last_write_time(preview.preview_path);
...
void c_preview::start(const std::filesystem::path& filepath) {
