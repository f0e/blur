...
bool gui::event_handler::process_event(const os::Event& ev) {
	switch (ev.type()) {
		...
		case os::Event::MouseDown: {
			keys::on_mouse_down(ev.position(), ev.button(), ev.modifiers(), ev.pointerType(), ev.pressure());
			return true;
		}

		case os::Event::MouseUp: {
			keys::on_mouse_up(ev.position(), ev.button(), ev.modifiers(), ev.pointerType());
			return true;
		}
...
bool gui::event_handler::generate_messages_from_os_events(bool rendered_last
	...
	if (rendered_last) // an animation is playing or something, so be quick
		timeout = 0.0;
	else // nothing's happening so take your time - but still be fast enough for the ui to update occasionally to see if
	     // it wants to render
		timeout = 1.f / 60;
