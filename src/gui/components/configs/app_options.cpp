...
void configs::theme_preview([[maybe_unused]] ui::Container& container) {
#ifdef BLUR_COLOR_THEMES
	// made-up settings, they don't stand for anything real. the open picker holds the input, so these are only
	...
	// only the elements that actually get tinted by the highlight color, otherwise it's not showing anything
	ui::add_slider(
